    ...
    <nav className="navbar is-fixed-top is-family-secondary is-transparent is-spaced" role="navigation">
      ...
      <div className={classnames("navbar-menu", isActive === true && "is-active")} data-target="navbar">
        <div id="navbar" className="navbar-end is-size-5 has-text-centered">
          <Link className="navbar-item" onClick={() => setActive(!isActive)} to="#work">WORK</Link>
          <Link className="navbar-item" onClick={() => setActive(!isActive)} to="#about">ABOUT</Link>
          <Link className="navbar-item" onClick={() => setActive(!isActive)} to="/blog">BLOG</Link>
          <Link className="navbar-item" onClick={() => setActive(!isActive)} to="#footer">CONTACT</Link>
        </div>
      </div>
    </nav>
  )
